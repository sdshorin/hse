...
EXAMPLE_MATRIX = [("A", 2, 2), ("B", 2, 3), ("C", 3, 4),
                  ("D", 3, 3), ("F", 4, 4), ("L", 2, 4)]

# TODO: USE DATABASE
# Для каждого пользователя храним его матрицы. {<user_id>: {"vars": {"A" =
# Matrix, "B" = Matrix}}}
...
# на команды /start и /help печатаем readme и выдаем ссылку на исходный
# код бота
...
# По команде /try создаем несколько случайных матриц заданного размера и
# содержании
...
def try_command(message):
    ...
    def create_random_matrix(x, y):
        return Matrix([[random.randrange(-10, 50)
    ...
    for matrix_data in EXAMPLE_MATRIX:
        ...
        bot.send_message(
            ...
            f"Матрица {matrix_data[0]} добавлена\nРазмер {matrix.size()}\n" +
            matrix.print_pretty())
...
def get_text_message(message):
    ...
    try:
        if message.text.find("=") != -1:
            ...
            matrix = eval_matrix_expression(
                message, check_expression(message.text))
...
def print_var(message, var):
    global user_data
    if var not in user_data[message.from_user.id]["vars"]:
...
def add_new_var_for_user(message):
    ...
    if not is_valid_matrix_name(new_var_name):
        raise BotException(
            "Некорректное имя переменной. Имя должно быть английской заглавной буквой")
    if not input_data:
        bot.send_message(
            message.from_user.id,
            f"Запущен интерактивный режим ввода матрицы. Вводите матрицу построчно, когда закончите - отправьте любое сообщение без чисел")
    ...
    if is_pure_matrix:
        ...
        user_data[message.from_user.id]["vars"][new_var_name] = Matrix(
            input_data)
        bot.send_message(
            message.from_user.id,
            f"Матрица {new_var_name} добавлена")
    else:
        # матрица создается из выражения
        new_matrix: Matrix = eval_matrix_expression(
            message, check_expression(input_data))
...
def eval_matrix_expression(message, expression):
    ...
    # В eval запрещены все встроенные функции, из переменных доступны только
    # матрицы
