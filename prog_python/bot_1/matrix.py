...
class Matrix:
    ...
    def __init__(self, arr) -> None:
        ...
        for line in self.arr:
            if len(line) != matrix_line_size:
                raise MatrixError(
                    self, self, f"Линия {line} должна состоять из {matrix_line_size} элементов")
    ...
    def __add__(self, other: Matrix):
        if self.size() != other.size():
            raise MatrixError(
                self, other, "Складывать можно только матрицы одинакового размера!")
        result = self.copy()
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, float) or isinstance(other, int):
            ...
            if self.size()[1] != other.size()[0]:
                raise MatrixError(
                    self, other, "Умножать матрицы можно только если a_m==b_n")
    ...
    def inverse(self):
        ...
        if not len(matrix.arr) or len(matrix.arr) != len(matrix.arr[0]):
            raise MatrixError(
                self,
                Matrix(inversed),
                "Обратную матрицу можно найти только у квадратной матрицы")
...
# 	print(a.inverse() * a)
        ...
        # return Matrix([[random.randrange(-10, 50) for _ in range(x)] for _ in
        # range(y)])
